...
class SequentialForwardSelector:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        included_feats = set()
        excluded_feats = set(range(X.shape[1]))
        ...
        for _ in (
            ...
            alpha = self.alpha / len(excluded_feats) if self.bonferroni else self.alpha
            for feat in excluded_feats:
                ...
                if p_value < alpha:
                    iter_results.append((subset, scores_b, p_value))
            ...
            included_feats = best_result[0]
            excluded_feats = excluded_feats.difference(included_feats)
            scores_a = best_result[1]

        self.selected_features_ = sorted(list(included_feats))
